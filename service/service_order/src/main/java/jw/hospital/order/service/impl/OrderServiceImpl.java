...
import jw.hospital.order.repository.ScheduleRepository;
...
public class OrderServiceImpl extends
    ...
    public Long saveOrder(String scheduleId, Long patientId) {
        ...
        //我们用下面的 JSONObject result = HttpRequestHelper.sendRequest(paramMap, signInfoVo.getApiUrl() + "/order/submitOrder");
        JSONObject result = HttpRequestHelper.sendRequest(paramMap, "http://localhost:9998/order/submitOrder");

        if(result.getInteger("code")==200) {
            // 更新mongodb排班预约人数
            Schedule schedule = scheduleRepository.findScheduleByHoscodeAndDepcode(orderInfo.getHoscode(), orderInfo.getDepcode());
            if (schedule != null) {
